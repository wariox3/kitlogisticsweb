...
--
-- Temporary table structure for view `sql_if_lista_facturas`
--

DROP TABLE IF EXISTS `sql_if_lista_facturas`;
/*!50001 DROP VIEW IF EXISTS `sql_if_lista_facturas`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE TABLE `sql_if_lista_facturas` (
  `IdFactura` int(11),
  `FhFac` datetime,
  `FhVenceFac` datetime,
  `IdCliente` varchar(11),
  ...
  `IdAsesor` int(11),
  `IdCentroCostos` int(11),
  `IdFormaPagoTercero` int(11),
  `Inactivo` tinyint(4),
  `ActualizadoWebServices` tinyint(4)
) ENGINE=MyISAM */;
SET character_set_client = @saved_cs_client;
...
--
-- Final view structure for view `sql_if_lista_facturas`
--

/*!50001 DROP TABLE IF EXISTS `sql_if_lista_facturas`*/;
/*!50001 DROP VIEW IF EXISTS `sql_if_lista_facturas`*/;
/*!50001 SET @saved_cs_client          = @@character_set_client */;
/*!50001 SET @saved_cs_results         = @@character_set_results */;
/*!50001 SET @saved_col_connection     = @@collation_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `sql_if_lista_facturas` AS select `facturas`.`IdFactura` AS `IdFactura`,`facturas`.`FhFac` AS `FhFac`,`facturas`.`FhVenceFac` AS `FhVenceFac`,`facturas`.`IdCliente` AS `IdCliente`,`facturas`.`Estado` AS `Estado`,`facturas`.`Notas` AS `Notas`,`facturas`.`TFlete` AS `TFlete`,`facturas`.`TManejo` AS `TManejo`,`facturas`.`TOtros` AS `TOtros`,`facturas`.`DctoComercial` AS `DctoComercial`,`facturas`.`BaseCcial` AS `BaseCcial`,`facturas`.`DctoFinanciero` AS `DctoFinanciero`,`facturas`.`BaseFin` AS `BaseFin`,`facturas`.`AntesDeDcto` AS `AntesDeDcto`,`facturas`.`Abonos` AS `Abonos`,`facturas`.`TotalFactura` AS `TotalFactura`,`facturas`.`Saldo` AS `Saldo`,`facturas`.`NroGuias` AS `NroGuias`,`facturas`.`NroPlanillas` AS `NroPlanillas`,`facturas`.`NroConceptos` AS `NroConceptos`,`facturas`.`Plazo` AS `Plazo`,`facturas`.`IdFormaPago` AS `IdFormaPago`,`facturas`.`IdTipoFactura` AS `IdTipoFactura`,`facturas`.`Exportada` AS `Exportada`,`facturas`.`IdEmpresa` AS `IdEmpresa`,`facturas`.`ValorEnLetras` AS `ValorEnLetras`,`terceros`.`IDTercero` AS `IDTercero`,`terceros`.`TpDoc` AS `TpDoc`,`terceros`.`RazonSocial` AS `RazonSocial`,`terceros`.`Nombre` AS `Nombre`,`terceros`.`Apellido1` AS `Apellido1`,`terceros`.`Apellido2` AS `Apellido2`,`terceros`.`Direccion` AS `Direccion`,`terceros`.`Telefono` AS `Telefono`,`terceros`.`IdCiudad` AS `IdCiudad`,`terceros`.`IdCliente` AS `IdNegociacion`,`terceros`.`Email` AS `Email`,`terceros`.`Celular` AS `Celular`,`terceros`.`Plazo` AS `PlazoTercero`,`terceros`.`DigitoVerificacion` AS `DigitoVerificacion`,`terceros`.`IdAsesor` AS `IdAsesor`,`terceros`.`IdCentroCostos` AS `IdCentroCostos`,`terceros`.`IdFormaPago` AS `IdFormaPagoTercero`,`terceros`.`Inactivo` AS `Inactivo`,`terceros`.`ActualizadoWebServices` AS `ActualizadoWebServices` from (`facturas` left join `terceros` on((`facturas`.`IdCliente` = `terceros`.`IDTercero`))) where (`facturas`.`Estado` <> 'D') order by `facturas`.`FhFac` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2014-10-07  7:53:58
